...
print(f"KMP Search: Matches at {m2}, Comparisons: {c2}")
print(f"Rabin-Karp Search: Matches at {m3}, Comparisons: {c3}")
text_large = ''.join(random.choices('ABCD',k=1000000))
patterns=['AB', 'ABCD', 'ABCDAB', 'ABCDABCD']
print(f"{'Pattern':>10} {'Naive Matches':>15} {'Naive Comps':>15} "
      f"{'KMP Matches':>15} {'KMP Comps':>15} "
      f"{'RK Matches':>15} {'RK Comps':>15}")
for p in patterns:
    _, c1 = naive_search(text_large, p)
    _, c2 = kmp_search(text_large, p)
    _, c3 = rabin_karp(text_large, p) 
    print(f"{p:>12} {c1:>10} {c2:>10} "  )   